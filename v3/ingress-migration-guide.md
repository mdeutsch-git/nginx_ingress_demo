...
metadata:
  ...
  annotations:
    ...
    # Scopes XFF trust to this pod only. Do NOT use meshConfig.defaultConfig.gatewayTopology
    # — that propagates to all sidecars and strips X-Forwarded-For at every inbound listener.
spec:
  containers:
    - name: istio-proxy
...
### Gateway isolation — automatic with Gateway API

When using `gatewayClassName: istio`, Istio provisions a **brand-new Deployment per `Gateway`
resource** (named `<gateway-name>-istio`). It does not reuse or modify `istio-ingressgateway`.
This means:

- The existing shared gateway and all traffic through it are completely unaffected
- No dedicated gateway setup is needed — you get isolation for free
- The `proxy.istio.io/config` annotation on the `Gateway` resource is propagated by Istio to the
  pod template of the provisioned Deployment, scoping XFF trust to this gateway only

```
istio-ingressgateway    ← existing, untouched
demo-gateway-istio      ← auto-provisioned from the Gateway resource, carries only these routes
```

This is a meaningful operational advantage over Option A's default path, where EnvoyFilters
targeting `istio: ingressgateway` affect all traffic through the shared gateway unless you
explicitly deploy a dedicated gateway.

### DestinationRule — more critical for Option B than Option A

Unlike `istio-ingressgateway` created by `istioctl install`,
this auto-provisioned pod does not inherit mTLS defaults from the installation profile. A `DestinationRule` with
...
> **Do not use `meshConfig.defaultConfig.gatewayTopology.numTrustedProxies`.** Despite being
> the approach shown in most Istio documentation, `defaultConfig` is mesh-wide — it applies
> to every proxy in the cluster, including sidecar inbound listeners. Every sidecar-injected
> pod will consume and strip `X-Forwarded-For` before the request reaches the application.
> The symptom: upstream services see no XFF header at all, regardless of which gateway sent
> the request.
>
> The correct approach is to scope `numTrustedProxies` to the gateway pod only via a
> `proxy.istio.io/config` pod annotation. This has no effect on any other proxy in the mesh.

**Option A — patch the ingressgateway Deployment:**
```bash
...
**Option A (dedicated gateway) — include annotation in the pod template directly:**
```yaml
metadata:
  annotations:
    inject.istio.io/templates: gateway
...
**Option B — add annotation to the Gateway resource:**

Istio propagates `proxy.istio.io/config` from the Gateway resource to the pod template of
the auto-provisioned Deployment:
```yaml
apiVersion: gateway.networking.k8s.io/v1
kind: Gateway
metadata:
...
> **Do not use `meshConfig.defaultConfig.gatewayTopology.numTrustedProxies`.** This setting
> propagates to every proxy in the mesh including sidecar inbound listeners, causing them to
> consume and strip `X-Forwarded-For` before it reaches the application. The correct approach
> is a `proxy.istio.io/config` annotation scoped to the gateway pod only.
...
```bash
# Option A (dedicated gateway) — annotation already in dedicated-gateway/deployment.yaml pod template
# No separate step needed if using the dedicated gateway path
```

```yaml
# Option B — annotation on the Gateway resource, propagated to the auto-provisioned Deployment
metadata:
  annotations:
    proxy.istio.io/config: '{"gatewayTopology": {"numTrustedProxies": 1}}'
```
